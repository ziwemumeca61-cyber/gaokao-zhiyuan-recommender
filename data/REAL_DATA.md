...
- 已内置（2025，物理+历史，来源各省教育考试院公布的成绩分段统计表，取官方关键分数段
  累计人数锚点、分段间对数插值）：**四川 / 广东 / 河南 / 河北 / 江苏**。
- 扩充：新增一份 CSV + 在 sources.json 登记出处即可，无需改代码；换算器的省份下拉会
  自动并入有种子的省份（即使该省不在录取数据里）。
- 暂未内置（待官方导入）：其余 3+1+2 省份（湖南/湖北/安徽/江西/重庆/福建/辽宁/贵州/
  广西/云南/陕西/山西/甘肃/黑龙江/吉林等）；**3+3 模式省份（北京/天津/山东/海南/上海/
  浙江）不分物理/历史**，为单张综合分段表，需单独适配，暂未接入。
